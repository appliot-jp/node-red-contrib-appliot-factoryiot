		...
		node.on('input', function (msg) {
			...
			if(msg.payload.hasOwnProperty("type")) {
				switch(msg.payload.type) {
					case "machine":
						query = (!APPLIOT.site) ? `?site=${APPLIOT.site}` : "";
					case "reason":
						getDatabase(msg.payload.type,query,(err,data) => {
							if(err) {
								console.log(err);
							} else {
				...
				for(let m of message) {
					if(rxdata.machine.debug === true) {
						...
						promise.then(() => {
							return new Promise((resolve) => {
								node.send([,,{
									topic: `v0/factory-iot/${APPLIOT.dbname}/${rxdata.machine.application}/data/latest/${m.id}`,
									payload: m,
									qos: 1,
								}]);
								resolve();
